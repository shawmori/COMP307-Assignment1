...
// assumes that the enclosing class has fields:
   ...
   List<OuterInstance> allOuterInstances;
   TreeBuilder tb;
   
   public Setup(String a, String test){
	   readDataFile(a);
	   setupInstances();
	   tb = new TreeBuilder(attNames, allOuterInstances, categoryNames);
	   readDataFile(test);
	   setupInstances();
	   new Tester(tb, allOuterInstances, categoryNames);
   }
   
   private void setupInstances(){
	   allOuterInstances = new ArrayList<OuterInstance>();
	   for(Instance i : allInstances){
		   allOuterInstances.add(new OuterInstance(i.getStringCategory(), i.getVals()));
	   }
   }
